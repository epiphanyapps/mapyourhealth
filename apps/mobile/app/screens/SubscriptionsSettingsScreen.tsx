 ...
 *
 * Screen for managing zip code subscriptions.
 * Lists current subscriptions and allows adding/removing zip codes.
 */
...
export const SubscriptionsSettingsScreen: FC<SubscriptionsSettingsScreenProps> = ({
  ...
  const [newSelections, setNewSelections] = useState<ZipCodeSelection[]>([])
  ...
  const renderEmpty = () => (
    <View style={$emptyContainer}>
      ...
      <Text style={$emptyText}>
        No zip codes subscribed yet.{"\n"}Add zip codes to monitor safety conditions.
      </Text>
    </View>
  )
    ...
    >
      ...
      <View style={$headerContainer}>
        <Text style={$countText}>
          {subscriptions.length} of {MAX_SUBSCRIPTIONS} zip codes
        </Text>
        {canAddMore && (
        ...
        <View style={$modalOverlay}>
          <View style={$modalContent}>
            <View style={$modalHeader}>
              <Text style={$modalTitle}>Add Zip Codes</Text>
            ...
            <Text style={{ color: theme.colors.textDim, marginBottom: 16 }}>
              You can add up to {remainingSlots} more zip code{remainingSlots !== 1 ? "s" : ""}.
            </Text>

            <ZipCodeSearch
              selectedZipCodes={newSelections}
              onSelectionChange={setNewSelections}
