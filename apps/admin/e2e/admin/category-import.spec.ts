...
test.describe('E2E-007: Category Import Sections', () => {
  ...
  test('shows all 3 category tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: /water quality/i })).toBeVisible();
    await expect(page.getByRole('tab', { name: /air pollution/i })).toBeVisible();
  ...
  test('Water Quality tab shows correct CSV format', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: /water quality/i })).toHaveAttribute(
      'data-state',
      'active'
    );

    // Verify category description is shown
    await expect(page.getByText(/import contaminant measurements/i)).toBeVisible();

    // Verify CSV format section shows required fields
    await expect(page.getByText('CSV Format')).toBeVisible();
    await expect(page.getByText(/city.*state.*country.*contaminantId.*value/i)).toBeVisible();
  ...
  test('Air Pollution tab shows correct CSV format', async ({ page }) => {
    ...
    // Verify tab is active
    await expect(page.getByRole('tab', { name: /air pollution/i })).toHaveAttribute(
      'data-state',
      'active'
    );
  ...
  test('switching tabs clears preview data', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /upload.*csv.*json/i })).toBeVisible();
  ...
  test('upload button exists and is enabled', async ({ page }) => {
    ...
    const uploadButton = page.getByRole('button', { name: /upload.*csv.*json/i });
